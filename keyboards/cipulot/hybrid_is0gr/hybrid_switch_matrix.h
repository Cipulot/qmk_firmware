...
typedef struct PACKED {
    ...
    uint16_t bottoming_reading;              // bottoming reading
} eeprom_ec_config_t;
...
typedef struct {
    ...
    uint16_t rescaled_mode_0_actuation_threshold;     // threshold for key press in mode 0 rescaled to actual scale
    uint16_t rescaled_mode_0_release_threshold;       // threshold for key release in mode 0 rescaled to actual scale
    uint16_t rescaled_mode_1_initial_deadzone_offset; // threshold for key press in mode 1 (initial deadzone) rescaled to actual scale
    uint16_t rescaled_mode_1_actuation_offset;        // offset for key press in mode 1 rescaled to actual scale
    uint16_t rescaled_mode_1_release_offset;          // offset for key release in mode 1 rescaled to actual scale
