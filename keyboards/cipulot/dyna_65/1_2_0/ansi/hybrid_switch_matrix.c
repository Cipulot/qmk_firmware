...
void select_amux_channel(uint8_t channel, uint8_t col) {
    ...
    // Small delay to allow the AMUX to stabilize after selection change
    wait_us(1);
}
...
// Improved noise rejection by ensuring the discharge pin is in high-Z state before selecting the AMUX channel and charging.
#ifdef OPEN_DRAIN_SUPPORT
    gpio_write_pin_high(DISCHARGE_PIN); // Release to Hi-Z
#else
    gpio_set_pin_input(DISCHARGE_PIN); // Set as input (Hi-Z)
#endif
    // Small delay to allow the pin state to stabilize after changing to Hi-Z
    wait_us(1);
